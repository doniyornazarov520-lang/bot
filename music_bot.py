...
# --- YOUTUBE'DAN YUKLASH (RENDER SERVERI UCHUN) ---
  ...
  try:
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
      ...
      if "entries" in info and len(info["entries"]) > 0:
        ...
        file_path = f"downloads/{video_data['id']}.mp3"

        return {
            "file_path": file_path,
            "title": video_data.get("title", "Unknown Title"),
            "performer": video_data.get("uploader", "Unknown Artist"),
        }
  except Exception as e:
    print(f"YouTube yuklashda xatolik: {e}")

  return None


# --- KANALGA TUSHGAN QO'SHIQLARNI AVTO-INDEKSASH ---
...
def index_channel_audio(message):
  if message.chat.id == CHANNEL_ID and message.audio:
    ...
    title = message.audio.title or "Noma'lum qo'shiq"
...
def build_search_keyboard(results, page=0):
  ...
  if page > 0:
    nav_btns.append(
        types.InlineKeyboardButton("⬅️ Oldingi", callback_data=f"page_{page-1}")
    )
  if end_idx < len(results):
    nav_btns.append(
        types.InlineKeyboardButton("Keyingi ➡️", callback_data=f"page_{page+1}")
    )
...
def search_music(message):
  ...
  # 1. BIRINCHI BO'LIB KANAL VA BAZADAN IZLAYDI
  if results:
  ...
  # 2. KANALDA BO'LMASA -> INTERNETDAN YUKLAB KANALGA HAM SAQLAYDI
  status_msg = bot.send_message(
      message.chat.id,
      f"🔎 **'{escape_markdown(message.text)}'** kanaldan topilmadi.\n"
      "🌐 Internetdan izlanmoqda va yuklanmoqda, kuting...",
      parse_mode="Markdown",
  )

  dl_result = download_from_youtube(message.text)

  if dl_result and os.path.exists(dl_result["file_path"]):
    try:
      # Kanalga yuklab saqlaymiz
      with open(dl_result["file_path"], "rb") as audio_file:
        sent_channel_msg = bot.send_audio(
            chat_id=CHANNEL_ID,
            audio=audio_file,
            title=dl_result["title"],
            performer=dl_result["performer"],
            caption=f"🌐 Avto-yuklandi: {dl_result['performer']} - {dl_result['title']}",
        )

      file_id = sent_channel_msg.audio.file_id
      ...
      new_music = {
          ...
          "performer": dl_result["performer"],
          "file_id": file_id,
          "search_text": f"{dl_result['performer']} {dl_result['title']}".lower(),
          "downloads": 1,
      }
      music_db.append(new_music)
      save_data(MUSIC_FILE, music_db)
      ...
      bot.send_audio(
          ...
          audio=file_id,
          caption=f"🎧 **{escape_markdown(dl_result['performer'])} - {escape_markdown(dl_result['title'])}**",
          parse_mode="Markdown",
      )

      if os.path.exists(dl_result["file_path"]):
      ...
      bot.edit_message_text(
          chat_id=message.chat.id,
          message_id=status_msg.message_id,
          text="❌ Qo'shiqni saqlashda xatolik yuz berdi.",
      )
  else:
    bot.edit_message_text(
        chat_id=message.chat.id,
        message_id=status_msg.message_id,
        text=f"❌ **'{escape_markdown(message.text)}'** bo'yicha hech qanday musiqa topilmadi.",
        parse_mode="Markdown",
    )
